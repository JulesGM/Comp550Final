...
flags.DEFINE_bool(
  "use_context",False,
  "The context option should be set either to 'True' or 'False'.")
...
class CommonsenseQAProcessor(DataProcessor):
  ...
  TRAIN_FILE_NAME = 'socialIQa_train.jsonl'
  DEV_FILE_NAME = 'socialIQa_dev.jsonl'
  TEST_FILE_NAME = 'socialIQa_test.jsonl'
  ...
  def _create_examples(self, lines, set_type):
    ...
    for line in lines:
      ...
      context = tokenization.convert_to_unicode(line['context'])
      answers = np.array([
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    #####context test
    result_1 = estimator.evaluate(input_fn=predict_input_fn, steps=1)

    output_test_1_file = os.path.join(FLAGS.output_dir, "pred_results_1.txt")
    with tf.gfile.GFile(output_test_1_file, "w") as writer:
      tf.logging.info("***** Pred results_1 *****")
      for key in sorted(result_1.keys()):
        tf.logging.info("  %s = %s", key, str(result_1[key]))
